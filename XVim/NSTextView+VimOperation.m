...
- (void)xvim_incrementNumber:(int64_t)offset{
    ...
    if (pos == NSNotFound) {
        if (![self xvim_currentNumber:&range]) {
            return;
        }
    } else {
    ...
    self.insertionPoint = range.location + repl.length;
}
...
- (BOOL)xvim_currentNumber:(NSRange *)range{
    ...
    if (x_end - x_start == 1) {
        ...
        if (end < s.length && [s characterAtIndex:end] == 'x') {
            ...
            if (end - x_start > 2) {
                ...
                *range = NSMakeRange(x_start, end - x_start);
                return YES;
            }
        }
    }
    ...
    if (x_end - x_start >= 1) {
        if (x_start > 2 && [s characterAtIndex:x_start - 1] == 'x' && [s characterAtIndex:x_start - 2] == '0') {
            *range = NSMakeRange(x_start - 2, x_end - x_start + 2);
            return YES;
        }
    }
    ...
    *range = NSMakeRange(n_start, n_end - n_start);
    return YES;
}
